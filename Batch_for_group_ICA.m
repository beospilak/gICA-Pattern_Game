...
%
% After entering the parameters, use icatb_batch_file_run(inputFile) or use Run_group_ICA.m script; 
...
icasso_opts.min_cluster_size = 16; % Minimum cluster size1
icasso_opts.max_cluster_size = 24; % Max cluster size. Max is the no. of components
...
for ii = 1:nsubs
    input_data_file_patterns{ii,1} =  fullfile(subs{ii},'*.nii');
end
...
refFiles = {which('ref_default_mode'), which('ref_left_visuomotor'), which('ref_right_visuomotor')};